...
class CartItem(models.Model):
    ...
    product_id = models.ForeignKey("products.Product", on_delete=models.CASCADE, related_name="cart_items")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["cart_id", "product_id"], name="unique_product_per_cart"),
        ]

    def __str__(self):
        return f"{self.quantity_item} x {self.product_id}"
    ...
    def save(self, *args, **kwargs):
        if not self.unit_price_item:
            self.unit_price_item = self.product_id.price
        self.full_clean()
