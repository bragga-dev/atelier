# atelier/apps/notifications/api/notification.py
"""
Rotas de Notificações.

- Usuário (cliente ou admin): visualiza, marca como lida e exclui as
...
from atelier.apps.core.exceptions import NotificationNotFound
from atelier.apps.core.permissions.auth_classes import AdminOnlyAuth, AllRolesAuth
from atelier.apps.core.schemas.deafult_schema import MessageOut, PageOut
from atelier.apps.core.utils.pagination import paginate_queryset
from atelier.apps.notifications.schemas.notification_schema import (
    MarkAllReadOut,
    NotificationOut,
    UnreadCountOut,
)
...
@router.get(
    "",
    response={200: PageOut[NotificationOut]},
    auth=AllRolesAuth(),
...
def list_notifications_router(request, page: int = 1, page_size: int = 20, unread_only: bool = False):
    ...
    qs = list_notifications_for_user(user_id=user.user_id, unread_only=unread_only)
    return 200, paginate_queryset(qs, page, page_size, NotificationOut.from_orm)


@router.get(
    "/unread-count",
    response={200: UnreadCountOut},
    auth=AllRolesAuth(),
    summary="Usuário obtém a contagem de notificações não lidas",
)
@ratelimit(key="user", rate="30/m", block=True)
def unread_count_router(request):
    user: User = request.auth
    return 200, UnreadCountOut(unread_count=unread_count_for_user(user_id=user.user_id))


@router.post(
    "/{notification_id}/read",
    response={200: NotificationOut, 404: MessageOut},
    auth=AllRolesAuth(),
...
def mark_as_read_router(request, notification_id: UUID):
    user: User = request.auth
    try:
        notification = mark_notification_as_read(user_id=user.user_id, notification_id=notification_id)
    except NotificationNotFound as e:
        return 404, {"detail": str(e)}
    return 200, NotificationOut.from_orm(notification)


@router.post(
    "/read-all",
    response={200: MarkAllReadOut},
    auth=AllRolesAuth(),
...
def mark_all_as_read_router(request):
    user: User = request.auth
    return 200, MarkAllReadOut(updated=mark_all_notifications_as_read(user_id=user.user_id))
...
def delete_notification_router(request, notification_id: UUID):
    user: User = request.auth
    try:
        delete_notification(user_id=user.user_id, notification_id=notification_id)
    except NotificationNotFound as e:
        return 404, {"detail": str(e)}
...
def list_all_notifications_router(
    ...
    qs = list_all_notifications_for_admin(admin_user_id=admin.user_id, target_user_id=user_id, read=read)
    return 200, paginate_queryset(qs, page, page_size, NotificationOut.from_orm)