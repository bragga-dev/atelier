...
import uuid
from atelier.apps.core.validators.validate_cep import validate_cep
...
class ProductShipping(models.Model):
    ...
    product_id = models.OneToOneField("products.Product", on_delete=models.CASCADE, related_name="shipping", )
    ...
    quantity = models.PositiveIntegerField(_("Quantidade por embalagem"), default=1, validators=[MinValueValidator(1, message=_("A quantidade deve ser pelo menos 1"))], help_text=_("Quantidade padrão de itens deste produto por embalagem, usada como base para a cotação de frete."))
    ...
    def __str__(self):
        return f"Frete de {self.product_id}"
